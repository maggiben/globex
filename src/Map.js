class Map {
  ...
  makeLand(atlas, {width, height}) {
    const {context, path} = this.makeProjectionContext({width, height});
    // Draw
    const feature = topojson.feature(atlas, atlas.objects.land);
    context.beginPath();
  ...
  makeCountries(atlas, {width, height}) {
    const {context, path} = this.makeProjectionContext({width, height});
    // Draw
    const feature = topojson.feature(atlas, atlas.objects.countries);
    context.beginPath();
